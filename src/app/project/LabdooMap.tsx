...
interface CountryDonationData {
  donations: number;
  name: string;
}

interface CountryReceivedData {
  locations: number;
  name: string;
}

async function loadCountryData(): Promise<{
  DONORS: Record<string, CountryDonationData>;
  RECEIVERS: Record<string, CountryReceivedData>;
}> {
  const response = await fetch("/country_totals.csv");
  ...
  const DONORS: Record<string, CountryDonationData> = {};
  const RECEIVERS: Record<string, CountryReceivedData> = {};

  (result.data as any[]).forEach((row) => {
    const iso = row.iso as string;
    const donated = Number(row.donated) || 0;
    const receivedVal = Number(row.received) || 0;

    if (donated > receivedVal) {
      DONORS[iso] = {
        name: iso,
        donations: donated,
      };
...
const { DONORS, RECEIVERS } = await loadCountryData();
...
async function loadLocations(): Promise<LocationPoint[]> {
  const response = await fetch("/map_points_out.csv");
  const csvText = await response.text();
  ...
  return (result.data as any[]).map((row) => ({
    ...
    lat: row.lat,
    lng: row.lng,
    type: row.type as "donor" | "receiver",
    count: row.count,
    label: row.label,
  }));
}

const POINTS = await loadLocations();
POINTS.pop(); // Remove the empty last row
